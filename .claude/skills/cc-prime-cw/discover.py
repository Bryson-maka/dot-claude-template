...
def load_project_config(base_dir: Path) -> dict[str, Any]:
    ...
    This file is generated by project_analyzer.py (auto-detection runs inline
    when this file is missing) and contains project-specific domain patterns
    and commands discovered from the codebase.

    If the file doesn't exist, falls back to auto-detection via project_analyzer.
    Returns empty dict if both methods fail.
    """
    ...
    # Auto-detect: call project_analyzer directly when no config file exists
    lib_path = str(base_dir / '.claude' / 'lib')
    try:
        sys.path.insert(0, lib_path)
        from project_analyzer import analyze_project
        analysis = analyze_project(base_dir)
        # Convert analysis.domain_suggestions to config format
        domains = []
        for d in analysis.domain_suggestions:
            domains.append({
                'name': d.name,
                'description': d.description,
                'patterns': d.patterns,
                'keywords': d.keywords,
                'report_sections': d.report_sections,
            })
        commands = {}
        if hasattr(analysis, 'command_suggestions'):
            commands = analysis.command_suggestions
        return {
            'domains': domains,
            'commands': commands,
            'auto_detected': True,  # Flag that this wasn't from a file
            'project_info': {
                'languages': dict(list(analysis.languages.items())[:10]),
                'frameworks': [f.name for f in analysis.frameworks if f.confidence >= 0.5],
            }
        }
    except ImportError:
...
def detect_monorepo(base_dir: Path, all_files: list[dict[str, Any]]) -> dict[str, Any]:
    """Detect if project is a monorepo with multiple independent packages."""
    package_indicators = {
        'package.json': 'node',
    ...
    # Find package indicators at different directory levels
    packages = []
    for f in all_files:
        name = Path(f['path']).name
        if name in package_indicators:
            parent = str(Path(f['path']).parent)
            if parent != '.':  # Skip root-level (that's normal)
                packages.append({
                    'path': parent,
                    'type': package_indicators[name],
                    'indicator': name,
                })
    ...
    return {
        ...
        'packages': packages if is_monorepo else [],
    }
...
def generate_manifest(
    ...
    source_file_count = sum(
        ...
        if not f['path'].endswith(('.md', '.json', '.yaml', '.yml', '.toml', '.ini', '.cfg'))
    )
